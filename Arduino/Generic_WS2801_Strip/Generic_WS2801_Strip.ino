...
struct state {
  uint8_t colors[3], bri = 100, sat = 254, colorMode = 1;
  bool lightState;
  int ct = 200, hue;
...
//settings
char lightName[LIGHT_NAME_MAX_LENGTH] = "Hue WS2801 strip";
uint8_t scene, startup, onPin = 4, offPin = 5;
// BE SURE TO CHECK YOUR PIXELS TO SEE WHICH WIRES TO USE!
uint8_t dataPin  = 13;    // Yellow wire on Adafruit Pixels, D5 on D1 mini
uint8_t clockPin = 14;    // Green wire on Adafruit Pixels, D7 on D1 mini

bool hwSwitch = false;
uint8_t rgb_multiplier[] = {100, 100, 100}; // light multiplier in percentage /R, G, B/

uint8_t lightsCount = 3;
uint16_t dividedLightsArray[50,50,50,50,50,50];

uint16_t pixelCount = 300, lightLedsCount;
...
uint32_t blending(float left[3], float right[3], uint8_t pixel) {
  uint8_t result[3];
  for (uint8_t i = 0; i < 3; i++) {
    float percent = (float) pixel / (float) (transitionLeds + 1);
    result[i] = (left[i] * (1.0f - percent) + right[i] * percent) / 2;
  }
  return Color((uint8_t)result[0], (uint8_t)result[1], (uint8_t)result[2]);
}

uint32_t convInt(float color[3]) {
  return Color((uint8_t)color[0], (uint8_t)color[1], (uint8_t)color[2]);
}

uint32_t convFloat(float color[3]) {
  return Color((uint8_t)color[0], (uint8_t)color[1], (uint8_t)color[2]);
}
        ...
        if (lightsCount > 1) {
          if (light == 0) {
            ...
            for (int p = 0; p < pixelCount; p++) { 
              strip.setPixelColor(p, convFloat(lights[light].currentColors));
            }
        }
        strip.show();
      }
...
//Converted
void ChangeWS2801(uint16_t newCount)
{
...
void setup() {
  ...
  ChangeWS2801(pixelCount);
  Serial.println("WS2801 initialized");

  Serial.println("WS2801 flashed");
  if (startup == 1) {
    for (uint8_t i = 0; i < lightsCount; i++) {
...
//Converted
uint32_t blendingEntert(float left[3], float right[3], float pixel) {
  uint8_t result[3];
  for (uint8_t i = 0; i < 3; i++) {
    float percent = (float) pixel / (float) (transitionLeds + 1);
    result[i] = (left[i] * (1.0f - percent) + right[i] * percent) / 2;
